    ...
    <script>
        ...
        window.addEventListener('beforeunload', function () {
            ...
            navigator.sendBeacon(
                '{{ route("games.abandon") }}',
                new Blob([JSON.stringify({ game_key: 'sequence-rush', _token: '{{ csrf_token() }}' })], { type: 'application/json' })
            );
        });
    </script>

    <div x-data="sequenceRush(window.__SEQ_INIT__)" x-init="init()" class="flex flex-col gap-10 max-w-3xl mx-auto relative overflow-hidden">
            ...
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-xs md:text-sm font-semibold text-[#564D4A]/50 leading-[1.3]">
                        <template x-if="!isSolved && !isFailed">
                            <span>Kies het juiste getal voor <span class="font-black text-[#564D4A]">?</span>. Max <span class="font-black text-[#564D4A]" x-text="puzzle.max_wrong"></span> fouten.</span>
                        </template>
        ...
        <div class="relative z-[1] w-full bg-white rounded-2xl p-8 border border-[#564D4A]/6">
            ...
            <div x-show="isSolved || isFailed" x-cloak>
                <div class="rounded-2xl border border-[#564D4A]/6 bg-[#F7F4F3] p-5">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs font-semibold text-[#564D4A]/55 leading-[1.3]"
                               x-text="isSolved ? 'Alle reeksen opgelost. Goed gedaan!' : 'Te veel fouten.'"></p>
                        </div>
                    </div>
                    ...
                    @include('games.partials.share-score-button', ['gameKey' => 'sequence-rush'])
            ...
            <div x-show="!isSolved && !isFailed" x-cloak>
                <div x-show="!started" x-cloak class="text-center py-10">
                    ...
                    <p class="mt-2 text-xs font-semibold text-[#564D4A]/55">Beantwoord alle 5 reeksen correct. Foute antwoorden geven straftijd.</p>
                ...
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-[1.2rem] font-extrabold text-[#564D4A]">Welk getal hoort op de ?</h2>
                        <p class="mt-1 text-xs font-semibold text-[#564D4A]/50 leading-[1.3]">
                            Kies snel. Fout antwoord geeft straftijd als je wint.
                        </p>
                    </div>

                    <span class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-[#5B2333]/10 text-[#5B2333] text-xs font-semibold">
                        <i class="fa-solid fa-layer-group"></i>
                        Vraag <span class="font-black" x-text="currentIdx + 1"></span>/<span x-text="puzzle.total"></span>
                    </span>
                </div>

                <div class="mt-6 rounded-2xl border border-[#564D4A]/6 bg-[#F7F4F3] p-6">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-[#564D4A]/45">Reeks</p>
                    ...
                    <p class="mt-3 text-[11px] font-semibold text-[#564D4A]/55">
                        Tip: de meeste reeksen volgen eenvoudige patronen (optellen, vermenigvuldigen, kwadraten, etc.).
                    </p>
                </div>
                </div>
            </div>
            ...
            return {
                ...
                startGame() {
                    ...
                    fetch('{{ route("games.mark-started") }}', {
                        ...
                        body: JSON.stringify({ game_key: 'sequence-rush' })
                    });
                    this.startTimer();
                },
                ...
                optionClass(opt) {
                    const base =
                        'bg-white border-[#564D4A]/6 text-[#564D4A] hover:border-[#5B2333]/40 hover:bg-[#F7F4F3] active:scale-[0.99]';
